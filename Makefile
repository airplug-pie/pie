...
tgz-pub: files-tgz-pub
	@echo " +++ $(DIR) : regle $@" ;
	@echo "     fabrication de l'archive airplug-$(DIR)-pub-`hostname`-`date +%d"-"%m"-"%Y`.tgz" ;
	@if  ! [ -e tgz-history.txt ] ; then \
		touch tgz-history.txt ; \
	fi ;
	@echo "$@ dans `pwd` sur `hostname` le `date +%A" "%d" "%B" "%Y" a "%k"h"%M":"%S`" >> tgz-history.txt
	@tar --no-recursion -czf airplug-$(DIR)-$(VERSION)-pub-`hostname`-`date +%d"-"%m"-"%Y`.tgz `cat files-tgz-pub.txt` ;


tgz-dev: files-tgz-dev
	@echo " +++ $(DIR) : regle $@" ;
	@echo "     fabrication de l'archive airplug-$(DIR)-$(VERSION)-dev-`hostname`-`date +%d"-"%m"-"%Y`.tgz" ;
	@if  ! [ -e tgz-history.txt ] ; then \
		touch tgz-history.txt ; \
	fi ;
	@echo "$@ dans `pwd` sur `hostname` le `date +%A" "%d" "%B" "%Y" a "%k"h"%M":"%S`" >> tgz-history.txt
	@tar --no-recursion -czf airplug-$(DIR)-$(VERSION)-dev-`hostname`-`date +%d"-"%m"-"%Y`.tgz `cat files-tgz-dev.txt` ;


tgz-skl: files-tgz-skl
	@echo " +++ $(DIR) : regle $@" ;
	@echo "     fabrication de l'archive airplug-$(DIR)-$(VERSION)-skl-`hostname`-`date +%d"-"%m"-"%Y`.tgz" ;
	@if  ! [ -e tgz-history.txt ] ; then \
		touch tgz-history.txt ; \
	fi ;
	@echo "$@ dans `pwd` sur `hostname` le `date +%A" "%d" "%B" "%Y" a "%k"h"%M":"%S`" >> tgz-history.txt
	@tar --no-recursion -czf airplug-$(DIR)-$(VERSION)-skl-`hostname`-`date +%d"-"%m"-"%Y`.tgz `cat files-tgz-skl.txt` ;
